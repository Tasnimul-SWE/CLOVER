...
def train_lassonet(
    ...
    model = LassoNet(input_dim=input_dim, num_classes=num_classes).to(device)

    optimizer = optim.Adam(model.parameters(), lr=lr)
    criterion = nn.CrossEntropyLoss()

    for _ in tqdm(range(num_epochs), desc="LassoNet training"):
        ...
        for xb, yb in loader:
            ...
            # L1 penalty on skip weights
            l1 = torch.sum(torch.abs(model.skip.weight))
    ...
    # feature importance from skip weights
    weights = model.skip.weight.detach().cpu().numpy()
...
# train lassonet
lasso_model, importance = train_lassonet(
    X_train,
    y_train,
...
# save results
runs.to_csv("lassonet_runs.csv",index=False)
summary.to_csv("lassonet_summary.csv",index=False)

pd.DataFrame({
    "variant_index":top100_idx,
    "variant_name":selected_variants
}).to_csv("lassonet_selected_variants.csv",index=False)
